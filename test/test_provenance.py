...
test_commit_details = [
    (
        "dataset_provenance",
        {
            "main": "https://raw.githubusercontent.com/weecology/retriever/master/test/raw_data/dataset-provenance/modified/dataset_provenance.csv"
        },
        {
            "original": "dataset-provenance-396e22.zip",
            "modified": "dataset-provenance-a69e22.zip",
        },
    )
]
...
def test_commit(script_file_name, modified_table_urls, expected_archives):
    ...
    script_module = get_script_module(
        os.path.join("raw_data/scripts/", script_file_name)
    )
    setattr(
        script_module,
        "_file",
        os.path.join(
            file_location, "raw_data/scripts/", "{}.json".format(script_file_name)
        ),
    )
    ...
    original_archive_exist = (
        True
        if os.path.isfile(os.path.join(test_dir, expected_archives["original"]))
        else False
    )
    modified_archive_exist = (
        True
